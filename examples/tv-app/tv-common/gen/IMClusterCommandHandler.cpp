...
namespace DefaultClusterHandler {

void DispatchServerCommand(app::Command * apCommandObj, chip::ClusterId aClusterId, CommandId aCommandId, EndpointId aEndpointId, TLV::TLVReader & aDataTlv)
{
    CHIP_ERROR TLVUnpackError    = CHIP_NO_ERROR;
    CHIP_ERROR TLVError          = CHIP_NO_ERROR;
    uint32_t currentDecodeTagId  = 0;
    ...
    chip::System::PacketBufferHandle packetBufferHandle = System::PacketBufferHandle::New(chip::System::PacketBuffer::kMaxSize);
    chip::System::PacketBufferTLVWriter pWriter;
    pWriter.Init(std::move(packetBufferHandle), true);
    pWriter.ImplicitProfileId = aDataTlv.ImplicitProfileId;    
    pWriter.StartContainer(chip::TLV::AnonymousTag, aDataTlv.GetContainerType(), outerContainerType);
    ...
    while ((TLVError = aDataTlv.Next()) == CHIP_NO_ERROR)
    {
        ...
        if (currentDecodeTagId < args.size() && args[currentDecodeTagId])
        {
            ChipLogProgress(Zcl, "Duplicate TLV tag %" PRIx32, TLV::TagNumFromTag(aDataTlv.GetTag()));
            TLVUnpackError = CHIP_ERROR_IM_MALFORMED_COMMAND_DATA_ELEMENT;
            break;
        ...
        {
            args.insert(args.begin() + currentDecodeTagId, true);
            validArgumentCount++;
        }

        //Copy the contents of the TLVReader into a TLVWriter
        TLVUnpackError = pWriter.CopyElement(aDataTlv);
    ...
    //Extract TLVWriter contents into packetBufferHandle
    pWriter.Finalize(&packetBufferHandle);
    if (CHIP_NO_ERROR == TLVError && CHIP_NO_ERROR == TLVUnpackError && argumentCount == validArgumentCount)
    {
        //Export packetBufferHandle contents into a byte buffer
        uint32_t length = packetBufferHandle->DataLength();
        uint8_t * data  = new uint8_t[length];
        packetBufferHandle->Read(data, length);

        wasHandled = emberAfUnknownClusterCallBack(apCommandObj, aClusterId, aCommandId, aEndpointId, data, length);
    }

    if (CHIP_NO_ERROR != TLVError || CHIP_NO_ERROR != TLVUnpackError || argumentCount != validArgumentCount || !wasHandled)
    {
        chip::app::CommandPathParams returnStatusParam = { aEndpointId,
                                                           0, // GroupId
        ...
        ChipLogProgress(Zcl,
                        "Failed to dispatch command, %" PRIu32 "/%" PRIu32 " arguments parsed, TLVError=%" CHIP_ERROR_FORMAT
                        ", UnpackError=%" CHIP_ERROR_FORMAT " (last decoded tag = %" PRIu32,
                        validArgumentCount, argumentCount, TLVError, TLVUnpackError, currentDecodeTagId);
        // A command with no arguments would never write currentDecodeTagId.  If
        // progress logging is also disabled, it would look unused.  Silence that
        // warning.
        UNUSED_VAR(currentDecodeTagId);
                                  ...
                                  chip::TLV::TLVReader & aReader, Command * apCommandObj)
{
    ...
    case Clusters::TargetNavigator::Id:
        ...
        break;
    default:
        // TODO: Add configuration enabled default cluster handling
        clusters::DefaultClusterHandler::DispatchServerCommand(apCommandObj, aClusterId, aCommandId, aEndPointId, aReader);
        break;
    }
exit:
