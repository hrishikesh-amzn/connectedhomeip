...
class CHIPDoSomethingClusterClusterDoSomethingResponseCallbackBridge : public Callback::Callback<DoSomethingClusterClusterDoSomethingResponseCallback>
{
public:
    CHIPDoSomethingClusterClusterDoSomethingResponseCallbackBridge(ResponseHandler handler, dispatch_queue_t queue): Callback::Callback<DoSomethingClusterClusterDoSomethingResponseCallback>(CallbackFn, this), mHandler(handler), mQueue(queue) {}

    ~CHIPDoSomethingClusterClusterDoSomethingResponseCallbackBridge() {};

    static void CallbackFn(void * context, uint8_t * data)
    {
        CHIPDoSomethingClusterClusterDoSomethingResponseCallbackBridge * callback = reinterpret_cast<CHIPDoSomethingClusterClusterDoSomethingResponseCallbackBridge *>(context);
        if (callback && callback->mQueue)
        {
            dispatch_async(callback->mQueue, ^{
                callback->mHandler(nil, @{
...
@interface CHIPDoSomethingCluster ()
@property (readonly) Controller::DoSomethingClusterCluster cppCluster;
@end

@implementation CHIPDoSomethingCluster
...
- (void)doSomething:(NSString *)data responseHandler:(ResponseHandler)responseHandler
{
    CHIPDoSomethingClusterClusterDoSomethingResponseCallbackBridge * onSuccess = new CHIPDoSomethingClusterClusterDoSomethingResponseCallbackBridge(responseHandler, [self callbackQueue]);
    if (!onSuccess) {
        responseHandler([CHIPError errorForCHIPErrorCode:CHIP_ERROR_INCORRECT_STATE], nil);
        return;
    }
    ...
    __block CHIP_ERROR err;
    dispatch_sync([self chipWorkQueue], ^{
        err = self.cppCluster.DoSomething(onSuccess->Cancel(), onFailure->Cancel(), chip::ByteSpan((const uint8_t*)[data dataUsingEncoding:NSUTF8StringEncoding].bytes, [data lengthOfBytesUsingEncoding:NSUTF8StringEncoding]));
    });
