...
{
    ChipLogProgress(Zcl, "DoSomethingResponse:");
    // Currently the generated code emits `uint8_t *` for CHAR_STRING, it needs to emits chip::ByteSpan
    // ChipLogProgress(Zcl, "  data: %.*s", data.size(), data.data());

    GET_CLUSTER_RESPONSE_CALLBACKS("DoSomethingClusterClusterDoSomethingResponseCallback");


    Callback::Callback<DoSomethingClusterClusterDoSomethingResponseCallback> * cb = Callback::Callback<DoSomethingClusterClusterDoSomethingResponseCallback>::FromCancelable(onSuccessCallback);
    cb->mCall(cb->mContext, data);
    return true;
}
